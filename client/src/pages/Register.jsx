      ...
      <section className="auth-premium-shell">
        <div className="auth-story">
          <div className="auth-story-copy">
            ...
            <p className="auth-story-lead">Create one PoultryDetect account to access the AI detector, dashboard and MongoDB-backed scan history across devices.</p>

            <div className="auth-feature-list">
              <div className="auth-feature"><span className="auth-feature-icon"><Icon type="cloud" /></span><div><strong>Persistent account</strong><small>Your profile and history are stored in MongoDB Atlas.</small></div></div>
              <div className="auth-feature"><span className="auth-feature-icon gold"><Icon type="shield" /></span><div><strong>Secure registration</strong><small>Passwords are hashed with bcrypt before storage.</small></div></div>
              <div className="auth-feature"><span className="auth-feature-icon teal"><Icon type="devices" /></span><div><strong>Ready anywhere</strong><small>Signed JWT sessions keep your workspace available across devices.</small></div></div>
            </div>
          ...
          <div className="auth-visual" aria-hidden="true">
            ...
            <div className="analysis-hud"><div className="analysis-hud-title">AI SCREENING</div><ul><li>Healthy</li><li>Coccidiosis</li><li>Salmonella</li><li>Newcastle</li></ul></div>
        ...
        <div className="auth-panel-premium">
          <div className="auth-card-premium">
            ...
            <p className="auth-card-subtitle">Enter your details once. Your MongoDB account will be created and your dashboard will open automatically.</p>
            {error && <div className="auth-error-premium">{error}</div>}

            <form onSubmit={handleSubmit}>
              <div className="premium-field"><label htmlFor="name">Full name</label><div className="premium-input-wrap"><Icon type="user" /><input id="name" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your full name" required /></div></div>
              ...
              <div className="premium-field"><div className="premium-label-row"><label htmlFor="password">Password</label><small>Minimum 6 characters</small></div><div className="premium-input-wrap"><Icon type="lock" /><input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" minLength={6} required /><button type="button" className="password-toggle" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? "Hide password" : "Show password"}><Icon type="eye" /></button></div></div>
              <div className="premium-field"><label htmlFor="confirm-password">Confirm password</label><div className="premium-input-wrap"><Icon type="lock" /><input id="confirm-password" type={showPassword ? "text" : "password"} autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Enter the same password again" minLength={6} required /></div></div>
              <button className="premium-auth-button" type="submit" disabled={loading}><span>{loading ? "Creating account…" : "Create account"}</span><span>→</span></button>
            </form>
